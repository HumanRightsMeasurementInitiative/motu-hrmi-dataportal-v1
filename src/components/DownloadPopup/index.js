...
import { flatMap, omitBy, mapKeys, mapValues, maxBy } from 'lodash'
...
function downloadFileWithContent(fileName, content, mimeType = 'text/plain') {
  ...
  downloadChart = () => {
    ...
    if (exploreBy === 'Geography') {
      if (currentCountryCode) {
        ...
        const fileName = `HRMIChart Human rights performance in ${currentCountryName}.svg`
        downloadFileWithContent(fileName, svgString, 'image/svg+xml')
      } else {
        // Makes no sense to download the entire country grid
      }
      ...
      const fileName = `HRMIChart Right to ${currentRight} in Region ${currentRegionName}.svg`
      downloadFileWithContent(fileName, svgString, 'image/svg+xml')
    }
  }
